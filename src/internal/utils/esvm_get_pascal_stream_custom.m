function fg = esvm_get_pascal_stream_custom(stream_params, dataset_params)
...
elseif exist('dataset_params','var') && isfield(dataset_params,'localdir') ...
      && ~isempty(dataset_params.localdir)
  CACHE_FILE = 1;
else
  dataset_params.localdir = '';
  CACHE_FILE = 0;
...
if ~isempty(stream_params.cls) && isfield(dataset_params,'clsimgsetpath') ...
...
elseif isfield(dataset_params,'imgsetpath') && isfield(stream_params,'stream_set_name')
  %% Load ids of all images in trainval that contain cls
  [ids, gt] = textread(sprintf(dataset_params.imgsetpath,...
                              stream_params.stream_set_name),...
                      '%s %d');
  ids = ids(gt==1);
  all_recs = cellfun2(@(x)sprintf(dataset_params.annopath,x),ids);
